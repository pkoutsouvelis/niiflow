...
|--------------------------|--------------------|------|
| `niiflow-core`           | `niiflow.core`     | MONAI + PyTorch Lightning training / finetuning |
...
`niiflow-core`, `niiflow-preproc`, and the **meta** `niiflow` package all contribute to the **`niiflow` namespace** (PEP 420): none of them ship `niiflow/__init__.py`. The meta wheel adds only `niiflow/__main__.py` (CLI) so `import niiflow` still works as a namespace import when any member is installed.

Install only what you need:

```bash
pip install niiflow-core
...
The workspace root depends on the `niiflow` meta package (workspace member), so a normal sync installs **niiflow-core**, **niiflow-preproc**, and **torch/monai/lightning** into `.venv` without needing `--all-packages`.
