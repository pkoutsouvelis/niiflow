...
from niiflow.preproc.functional.array import croppad as _array_croppad
...
def bbox_from_mask(mask: ANTsImage, pad: int = 0) -> tuple[slice, ...]:
    """Bounding box of an ANTsImage mask, as a tuple of voxel-index slices."""
    ensure_ants_image(mask, name="mask")
    return _array_croppad.bbox_from_mask(mask.numpy(), pad=pad)
...
def crop_to_range(
    ...
    out_array = _array_croppad.crop_to_range(image_array, ranges)
...
def crop_to_mask(
    ...
    out_array, crop_ranges = _array_croppad.crop_to_mask(
        image_array, mask=mask_array, pad=pad
    )
...
def center_crop(
    ...
    out_array, crop_ranges = _array_croppad.center_crop(
        image_array, shape=shape, mask=mask_array
    )
    voxel_offset = _voxel_offset_from_ranges(crop_ranges)
    out_image = reconstruct_ants_image(out_array, image, voxel_offset=voxel_offset)
    return out_image, crop_ranges
...
def pad_to_range(
    ...
    image_array, _ = ants_to_numpy_with_metadata(image)
    out_array = _array_croppad.pad_to_range(
        image_array,
        ranges,
        mode=mode,
...
def center_pad(
    ...
    out_array, pad_ranges = _array_croppad.center_pad(
        image_array,
        shape=shape,
        mask=mask_array,
        mode=mode,
