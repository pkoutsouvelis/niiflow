...
class TestBlockingQueueHandler:
    def test_drops_quickly_without_handle_error_storm(self) -> None:
...
class TestBoundedQueueListener:
    def test_saturated_spawn_worker_continues_and_listener_stops(self) -> None:
...
class TestParallelLogging:
    def test_uses_spawn_bounded_queue_and_stops_twice(
        self,
        tmp_path: Path,
    ) -> None:
        mp_context = multiprocessing.get_context("spawn")
        ...
        assert (
            "1834 worker log records suppressed due to logging backpressure" in main_log
        )
